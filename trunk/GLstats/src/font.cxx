  ...
  : textureSize(textureSize)
{
  ...
  if(error) {
    ...
    throw BasicException(mess.append(filename));
  }

  //Set char size
  error = FT_Set_Pixel_Sizes(face, pixelSize, pixelSize);
  if(error)
    throw BasicException("FontGL: Can't set char size.");
...
FontGL::generateTexture()
{
  ...
  for(int c = 0; c < 255; c++) {
    ...
    FT_Error error = FT_Load_Glyph(face, FT_Get_Char_Index(face, c),
                                   FT_LOAD_DEFAULT);
    if(error)
      throw BasicException("FontGL: Can't load gyph.");

    error = FT_Get_Glyph(face->glyph, &glyph);
    if(error)
      throw BasicException("FontGL: Can't get gyph.");

    error = FT_Glyph_To_Bitmap(&glyph, FT_RENDER_MODE_NORMAL, 0, 0);
    if(error)
      throw BasicException("FontGL: Can't convert gyph to bitmap.");
...
FontGL::initLibrary()
{
  ...
  if (error)
    throw BasicException("Can't initializate Freetype");
}
