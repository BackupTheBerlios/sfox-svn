...
FontGL::generateTexture()
{
  ...
  Texture *texCurrent = new Texture(textureSize, textureSize,
                                     Texture::TEXTURE_2D, Texture::ALPHA|
                                     Texture::UNSIGNED_BYTE|Texture::NEAREST);
  ...
  for(int c = 0; c < 255; c++) {
    ...
    if(v+bmp.rows >= textureSize) {
      u = v = maxHeight = 0;
      texCurrent = new Texture(textureSize, textureSize, Texture::TEXTURE_2D,
                               Texture::ALPHA|Texture::UNSIGNED_BYTE|
                               Texture::NEAREST);
      textures.push_back(texCurrent);
      texCurrent->bind();
    }
    ...
    texCurrent->subImage(bmp.buffer, u, v, bmp.width, bmp.rows, Texture::ALPHA);
    u += bmp.width;
...
FontGL::drawChar(float x, float y, int c)
{
  ...
  if(currentTexture != tmp->texture) {
    ...
    textures[currentTexture]->setTexEnv(GL_TEXTURE_ENV_MODE, GL_BLEND);
  }
  x += tmp->left;
...
FontGL::testRender()
{
  ...
  textures[0]->bind();
  glEnable(GL_BLEND);
  textures[0]->setTexEnv(GL_TEXTURE_ENV_MODE, GL_BLEND);
  glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
